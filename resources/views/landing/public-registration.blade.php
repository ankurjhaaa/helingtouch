...
@section('content')
    ...
    <div class="max-w-md  mx-auto mt-30 bg-white p-6 shadow rounded">
        ...
        <form action="{{ route('public.register.apply') }}" method="POST">
            @csrf
            <input type="text" name="name" class="w-full mb-3 border rounded px-3 py-2 " placeholder="Name" value="{{ old('name') }}" >
            @error('name')
            ...
            <div class="text-red-500 text-sm mb-2">{{ $message }}</div>
            @enderror
             <input type="number" name="phone" class="w-full mb-3 border rounded px-3 py-2" placeholder="phone" >
            @error('phone')
            <div class="text-red-500 text-sm mb-2">{{ $message }}</div>
            @enderror
        ...
        <p class="text-sm mt-4 text-center">Already registered? <a href="{{ route('showlogin') }}"

...
<body class="text-gray-800">
    ...
    <!-- Top Navbar -->
    <nav
        class="bg-gradient-to-r from-blue-900 via-blue-800 to-blue-700 text-white shadow-lg px-4 py-3 flex justify-between items-center sticky top-0 z-50">
        <!-- Left: Toggle Button and Title -->
        <div class="flex items-center space-x-2">
            <!-- Sidebar Toggle Button (Mobile) -->
            <button id="sidebar-toggle"
                class="sm:hidden text-white bg-blue-600 p-2 rounded-md hover:bg-blue-700 transition-colors duration-200">
        ...
        <div class="relative">
            <button id="user-menu-button" class="flex items-center space-x-2 focus:outline-none group">
                <img src="{{ Auth::check() && Auth::user()->photo && file_exists(storage_path('app/public/' . Auth::user()->photo)) ? asset('storage/' . Auth::user()->photo) : 'https://picsum.photos/40' }}"
                    alt="Profile"
                    class="w-8 h-8 rounded-full object-cover border-2 border-blue-300 shadow-md group-hover:scale-110 transition-transform duration-300">
                <span
                    class="text-xs sm:text-sm font-semibold text-blue-100 group-hover:text-white hidden sm:block">{{ Auth::user()->name }}</span>
                <svg class="w-4 h-4 text-blue-200 group-hover:text-white" fill="none" stroke="currentColor"
            ...
            <div id="user-dropdown"
                class="hidden absolute right-0 mt-2 w-36 sm:w-40 bg-white rounded-lg shadow-xl py-2 z-50 border border-gray-100">
                <a href="{{ route('admin.profile') }}"
                    class="flex items-center px-3 py-2 text-xs sm:text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-800">
                ...
                <a href="{{ route('auth.logout') }}"
                    class="flex items-center px-3 py-2 text-xs sm:text-sm text-red-600 hover:bg-red-50 hover:text-red-800">
